...
export const surveyZodSchema = z
	.object({
		surveyCode: z
			.string()
			.length(
...
// REVIEW: Do we need swagger for schemas?
...
export const updateSurveySchema = surveyZodSchema
	.pick({
		responses: true,
		isCompleted: true
	})
...
// REVIEW: Auto formatter. INstall prettier, and turn on autoformatter. Then verify if it picks up .prettierrc.json
export const readSurveyByObjectIdSchema = z
	.object({
		_id: z
			.string()
...
export const readSurveySchema = surveyZodSchema
	.partial()
	.meta({ model: Survey });

export const findParentByChildCodeSchema = z
	.object({
