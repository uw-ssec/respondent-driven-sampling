...
import { saveAuthToken, getObjectId, hasAuthToken, getAuthToken } from '@/utils/authTokenHandler';
...
export const useAuth = () => {
  ...
  const fetchUserContext = async (userObjectId: string) => {
    try {
      ...
      const surveysResponse = await fetch(`/api/v2/surveys?createdByUserObjectId=${userObjectId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      const surveys = await surveysResponse.json();
      const latestLocationObjectId = surveys.data[0]?.locationObjectId ?? user.data.locationObjectId;
