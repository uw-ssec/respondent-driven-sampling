...
import { useSurveyStore, useAuthStore } from '@/stores';
import { useGeolocated } from 'react-geolocated';

import { useApi, useAbility } from '@/hooks';
  ...
  const {
	...
	useEffect(() => {
		const init = async () => {
			...
			if (codeInUrl && codeInUrl !== getReferredByCode()) {
				...
				const surveyData = await surveyService.fetchSurveyByReferralCode(codeInUrl);
				...
				if (surveyData.data.length == 1) {
					...
					const parentSurvey = await surveyService.fetchParentOfSurveyCode(codeInUrl);
				...
				if (!ability.can(ACTIONS.CUSTOM.CREATE_WITHOUT_REFERRAL, SUBJECTS.SURVEY)) {
					alert('You do not have permission to create a survey without a referral code.');
					navigate('/apply-referral');
					return;
				}
			}
	...
	const attachSurveyEventHandlers = (survey: Model | null) => {
		...
		survey.onComplete.add(async sender => {
			...
			try {
				const result = await surveyService.updateSurvey(getObjectId()!, {
					responses: surveyData.responses,
					isCompleted: true
				});
				if (result) {
